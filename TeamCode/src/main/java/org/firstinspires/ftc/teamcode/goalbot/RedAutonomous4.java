...
@Autonomous(name = "red auto4", group = "red")
public class RedAutonomous4 extends GoalBotAutonomous {
...
////        telemetry.update();
        ...
        if (rings == Rings.ONE) {
            driveToPosition(36,6, bot.getPose().x - 6,53,shootingAngle,2,1);
        } else if (rings == Rings.ZERO){
            driveToPosition(36,6, bot.getPose().x + 7,24,shootingAngle,2,1);
        } else {
...
//            driveToPosition(36, 6, 36, 32, 180, 2, 1);
        ...
        if(rings == Rings.ONE) {
            //driveToPosition(new MotionProfile(8, 48, 25), X_SHOOT, Y_SHOOT, shootingAngle, 1);
