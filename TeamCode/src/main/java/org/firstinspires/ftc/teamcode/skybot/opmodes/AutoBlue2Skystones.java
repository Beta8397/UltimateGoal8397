 ...
 */
@Disabled
@Autonomous(name = "Blue 2 Skystones", group = "No")
public class AutoBlue2Skystones extends SkyBotAutonomous {
    ...
    public void runLoggingOpmode() {
        ...
        driveDirectionGyro(FAST_DRIVE_SPEED, 180, 0, new Predicate() {
            @Override
            public boolean isTrue() {
                return bot.frontDistSensor.getDistance(DistanceUnit.CM) < 60 && robotXYTheta[0] < FIELD_WIDTH * .35;
            }
        });

        turnToHeadingGyro(90,3, 0.3f);
        ...
        while(opModeIsActive() && et.milliseconds() < 1000) {
            ...
            robotXYTheta = VuforiaNavigator.getX_Y_Theta_FromLocationTransform(robotLocationTransform);
            success = true;
            break;
        }
        ...
        final float targetX = (skyStonePosition == SkyStonePosition.RIGHT?8:(skyStonePosition == SkyStonePosition.CENTER?16:24)) * 2.54f + BOT_LENGTH / 2 + 2.54f;

        turnToHeadingGyro(0, 3, 0.3f);
